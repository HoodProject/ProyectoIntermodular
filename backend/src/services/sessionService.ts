...
import type { EndSessionInput, StartSessionInput } from '../validators/sessionValidator.js';
...
function calculateCost(minutos: number, tarifaPorHora?: number): number {
...
export async function startSession(payload: StartSessionInput): Promise<Session> {
...
export async function endSession(payload: EndSessionInput): Promise<Session> {
  const cierre = dayjs();
  ...
  const sessionRow = Array.isArray(rows) ? (rows[0] as any) : null;
  if (!sessionRow) {
